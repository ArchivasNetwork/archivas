#!/usr/bin/env bash
# setup-seed3.sh - Deploy Archivas seed3 node (seed3.archivas.ai)
# Based on seed2 setup.sh with domain changed to seed3
...
DOMAIN="seed3.archivas.ai"  # Changed from seed2
NETWORK="archivas-devnet-v4"

echo "🌾 Archivas Seed3 Setup"
echo "======================"
echo "Domain: $DOMAIN"
...
mkdir -p "$DATA_DIR"
mkdir -p "$LOG_DIR"
sudo mkdir -p /var/www/seed3
echo "   ✅ Directories created"
echo ""

# Create systemd service
echo "5️⃣  Creating systemd service..."
sudo tee /etc/systemd/system/archivas-node-seed3.service >/dev/null <<EOF
[Unit]
Description=Archivas Node (seed3)
...
Restart=always
RestartSec=3
StandardOutput=append:$LOG_DIR/archivas-node-seed3.log
StandardError=append:$LOG_DIR/archivas-node-seed3.log
LimitNOFILE=65535
...
sudo systemctl enable archivas-node-seed3
echo "   ✅ Systemd service created"
echo ""

# Create Nginx CORS snippet (if not exists)
echo "6️⃣  Creating Nginx CORS configuration..."
if [ ! -f /etc/nginx/snippets/archivas-cors.conf ]; then
...
# Create rate limiting configuration (if not exists)
echo "7️⃣  Creating rate limiting configuration..."
if [ ! -f /etc/nginx/conf.d/archivas-ratelimit.conf ]; then
  sudo tee /etc/nginx/conf.d/archivas-ratelimit.conf >/dev/null <<'EOF'
# Rate limiting zones for seed nodes
...
echo "   ✅ Rate limiting configuration ready"
echo ""
...
echo "🔟 Starting node..."
sudo systemctl restart archivas-node-seed3
sleep 5

# Check if node is running
if systemctl is-active --quiet archivas-node-seed3; then
  echo "   ✅ Node started successfully"
else
  echo "   ⚠️  Node failed to start. Check logs:"
  echo "   sudo journalctl -u archivas-node-seed3 -n 50"
  exit 1
fi
echo ""
...
echo ""
echo "✅ Seed3 deployment complete!"
echo ""
echo "Next steps:"
echo "1. Configure DNS: $DOMAIN → $(curl -s ifconfig.me || echo 'YOUR_SERVER_IP')"
echo "2. Verify: curl https://$DOMAIN/chainTip | jq"
echo "3. Check logs: sudo journalctl -u archivas-node-seed3 -f"
echo "4. Update Explorer and SDK to use $DOMAIN as tertiary RPC"
echo ""
echo "Node status:"
systemctl status archivas-node-seed3 --no-pager | head -10
